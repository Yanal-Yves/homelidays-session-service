...
// 
// This file has been patched to match Session.Content requirements.
//
...
namespace VCUE
{
	...
	public :
		...
		static HRESULT copy(destination_type* pTo, const source_type* pFrom)
		{
			// patch YFA : For Each Session.Content should return each key instead of each value
			_variant_t key;
			key = pFrom->first;
			HRESULT hr;
			hr = GenericCopy<destination_type, pseudosource_type>::copy(pTo, &key);
